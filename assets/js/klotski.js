...
let NO_LR_MIRROR_ALLOW = true

let MAX_MOVE_DIRECTION = 4
let MAX_WARRIOR_TYPE = 5

let HRD_GAME_ROW = 5
let HRD_GAME_COL = 4
let HRD_BOARD_WIDTH = HRD_GAME_COL + 2
let HRD_BOARD_HEIGHT = HRD_GAME_ROW + 2

let CAO_ESCAPE_ROW = 3
let CAO_ESCAPE_COL = 1

let BOARD_CELL_EMPTY = 0
let BOARD_CELL_BORDER = -1
...
function Klotski() {
  ...
  function getZobristHashUpdate(gameState, blockIdx, dirIdx, isMirror) {
    ...
    var dx = isMirror ? -1 : 1
    var dir =
      directions[isMirror && dirIdx % 2 === 1 ? (dirIdx + 2) % 4 : dirIdx]
    var x = dir.x
    var y = dir.y
  ...
  function canBlockMove(state, blockIdx, dirIdx) {
    ...
    switch (block.type) {
      case HT_BLOCK:
        canMove =
          state.board[block.row + dir.y + 1][block.col + dir.x + 1] ==
          BOARD_CELL_EMPTY
        break
      case HT_VBAR:
  ...
  this.solve = function(blocks, options) {
    if (
      options &&
      options.hasOwnProperty('useMirror') &&
      typeof options.useMirror === 'boolean'
    ) {
